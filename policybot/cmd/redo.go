...
func redoCmd() *cobra.Command {
	...
	cmd, _ := cmdutil.Run("redo", "Run the GitHub+ZenHub state redo", 0,
		cmdutil.ConfigPath|cmdutil.ConfigRepo|cmdutil.ZenhubToken|cmdutil.GitHubToken|cmdutil.GCPCreds, func(reg *config.Registry, secrets *cmdutil.Secrets) error {
			return runRedo(reg, secrets, syncFilter)
		})
...
func runRedo(reg *config.Registry, secrets *cmdutil.Secrets, syncFilter string) error {
	...
	return mgr.Redo(context.Background(), flags, false)
}
